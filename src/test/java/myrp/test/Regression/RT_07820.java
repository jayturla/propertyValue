...
public class RT_07820 extends TestInitReference {
	{
		System.setProperty("atu.reporter.config", "../myrp-automation/conf/atu.properties");
	}
	
	String testCase = "RT_07820";
		
	@Test(description="RT_07820", dataProvider = "Data-Provider-Function")
	public void RT_07927_to_RT_07930(Class clzz, String[] input) {
		...
		try {
			...
			if(myrp.SubscriptionReport()){
				ATUReports.add(input[1],"SUBSCRIPTION REPORTS",input[2], input[3], true);//pass
			}else{
				ATUReports.add(input[1],"SUBSCRIPTION REPORTS",input[2], input[4], true);//fail
			}
	...
	public Object[][] parameterIntTestProvider() throws IOException{
		Object[][] data = null;
		ReadXlsData rxd = new ReadXlsData("../myrp-automation/test-data/RT_07820.xls");
		data = rxd.getData();
		return data;
	}
