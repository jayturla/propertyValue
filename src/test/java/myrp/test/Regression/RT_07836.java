...
public class RT_07836 extends TestInitReference {
	{
		System.setProperty("atu.reporter.config", "../myrp-automation/conf/atu.properties");
	}
	
	String testCase = "RT_07836";
		
	@Test(description="RT_07836", dataProvider = "Data-Provider-Function")
	public void RT_07927_to_RT_07930(Class clzz, String[] input) {
		...
		try {
			...
			if(myrp.fastPartner()){
				ATUReports.add(input[1],"FAST PARTNER",input[2], input[3], true);//pass
			}else{
				ATUReports.add(input[1],"FAST PARTNER",input[2], input[4], true);//fail
			}
	...
	public Object[][] parameterIntTestProvider() throws IOException{
		Object[][] data = null;
		ReadXlsData rxd = new ReadXlsData("../myrp-automation/test-data/RT_07836.xls");
		data = rxd.getData();
		return data;
	}
