...
public class SuburbSalesHistoryReportUtil extends FunctionReference {
	
	private String[] input = null;
	private ATUUtil atu = new ATUUtil();
	
	public SuburbSalesHistoryReportUtil(String[] i) {
		input = i;
	
	}
	
	//check if can navigate to shopping cart screen when go to cart button is clicked after adding to cart
	public boolean shopCartScreen(int steps, int inputVal, int expected, int actualPass, int actualFail, int suburb, boolean withATU) throws Exception {
		...
		waitForElementPresent(xpath(PVObjectReferenceSmoketest.suburbAddToCart));
		Assert.assertTrue(isElementPresent(xpath(PVObjectReferenceSmoketest.suburbAddToCart)));
		Thread.sleep(2000);
		click(xpath(PVObjectReferenceSmoketest.suburbAddToCart));
