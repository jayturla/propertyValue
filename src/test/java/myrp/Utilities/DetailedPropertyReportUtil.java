...
public class DetailedPropertyReportUtil extends FunctionReference {
	
	private String[] input = null;
	private ATUUtil atu = new ATUUtil();
	
	public DetailedPropertyReportUtil(String[] i) {
		input = i;
	
	}
	
	//check if can navigate to shopping cart screen when go to cart button is clicked after adding to cart
	public boolean shopCartScreen(int steps, int inputVal, int expected, int actualPass, int actualFail, int property, boolean withATU) throws Exception {
		boolean passed = false;
		
		type(xpath(PVObjectReferenceSmoketest.slasBoxSC), input[property]);
		clickselectSuggestion(0,0,0,0,false);

		waitForElementPresent(xpath(PVObjectReferenceSmoketest.detailedAddToCart));
		Assert.assertTrue(isElementPresent(xpath(PVObjectReferenceSmoketest.detailedAddToCart)));
		Thread.sleep(2000);
		click(xpath(PVObjectReferenceSmoketest.detailedAddToCart));
