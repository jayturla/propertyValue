...
public class SalesHistoryReportUtil extends FunctionReference {
	
	private String[] input = null;
	private ATUUtil atu = new ATUUtil();
	
	public SalesHistoryReportUtil(String[] i) {
		input = i;
	...
	public boolean shopCartScreen(int steps, int inputVal, int expected, int actualPass, int actualFail, int property, boolean withATU) throws Exception {
		...
		Assert.assertTrue(isElementPresent(xpath(PVObjectReferenceSmoketest.salesAddToCart)));
		Thread.sleep(2000);
		click(xpath(PVObjectReferenceSmoketest.salesAddToCart));
